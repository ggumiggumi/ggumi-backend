...
public class BookDetailService implements BookDetailRepository {
	...
	public Long setHate(Long key, Long value) {
		...
		if (Integer.parseInt(Objects.requireNonNull(redisTemplate.opsForValue().get(TOTAL_LIKES + key)).toString())
			> 0) {
	...
	private void updateRecommendBooks(History history) {
		...
		double childEiMax = Math.min(1.0, history.getEI() + range);

		double childFtMin = Math.max(0.0, history.getFT() - range);
		double childFtMax = Math.min(1.0, history.getFT() + range);

		double childPjMin = Math.max(0.0, history.getPJ() - range);
		double childPjMax = Math.min(1.0, history.getPJ() + range);

		double childSnMin = Math.max(0.0, history.getSN() - range);
		double childSnMax = Math.min(1.0, history.getSN() + range);
